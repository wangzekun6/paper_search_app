# PaperSearch：面向自然语言论文检索的 LLM-first 系统

总述：本仓库中的代码、数据样例、评测产物、演示素材与历史归档文件共同构成了 PaperCompass 论文原型从构建运行、效果验证到答辩展示的完整交付。

## 1. 项目简介

PaperSearch 是一个面向学术论文检索场景的智能检索系统。系统以自然语言查询为输入，围绕“用户意图理解、候选召回、query-paper match、重排解释、论文详情展示”构建完整闭环，旨在提升传统关键词检索在意图表达、多轮追问与结果可解释性方面的能力。
...
从工程实现角度看，PaperSearch 并非单一算法脚本，而是一套较完整的系统原型，覆盖了前端界面、命令行入口、LLM 接入、检索链路编排、语义卡缓存、后台补全、评估产物输出等多个层面，适合作为“基于大语言模型的论文检索系统”相关毕设项目进行展示与说明。
...
从功能定位看，PaperSearch 不是一个简单的论文搜索页面，而是一套 LLM-first 的论文检索系统。其主要特征包括：
...
本系统要求所有用户输入默认先经过 LLM 意图分析，而不是仅在个别环节调用模型。这样做的原因在于：如果核心意图理解仍由 heuristic 主导，那么系统的核心主张将无法成立。PaperSearch 将 LLM 置于正式主路径，用于识别以下信息：
...
PaperSearch 以论文检索为应用场景，围绕“LLM 意图理解、多路召回、query-paper match、重排解释与完整论文详情展示”构建了一套较完整的智能检索系统。
