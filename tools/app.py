...
def main() -> None:
    ...
    if stats.get("papers", 0) <= 0:
        st.error("Current database file exists but contains no papers. Run `python papercompass.py build` in `tools/` first.")
        st.caption(f"Current database: {relative_to_project(get_default_db_path())}")
        return
