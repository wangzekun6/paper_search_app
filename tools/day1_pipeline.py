...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "day1_outputs"
DEFAULT_DATA_ROOT = DATASET_DIR
SCHEMA_PATH = TOOLS_DIR / "day1_schema.sql"
...
def parse_args() -> argparse.Namespace:
    """解析 Day 1 命令行参数，决定样本来源和输出目录。"""

    parser = argparse.ArgumentParser(description="Run the Day 1 paper contract pipeline.")
    parser.add_argument(
        "--data-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory used to write Day 1 outputs.",
    )
    parser.add_argument(
        "--validation-ids",
        nargs="*",
...
def build_records_for_paper(path: Path, data_root: Path) -> Tuple[RawPaperRecord, PaperIndexRecord, List[PaperSectionRecord], PaperSemanticCard]:
    """
    从单篇论文 JSON 一次性构建 Day 1 需要的全部标准对象。
...
PaperSemanticCard
- 仅冻结字段名，不在 Day 1 生成内容。
- 存储时序列化为 semantic_card_json，Day 3 之后再填充。
...
结论
- Day 1 所需对象协议、字段分工、SQLite schema 和 3 条样例索引记录已经在项目内落地，可直接作为 Day 2 全量入库的基础。
"""
    write_text(output_dir / "day1_feedback.txt", content)
...
def initialize_database(
    ...
    """
    按 Day 1 schema 写入样例数据库。

    这里写入的是少量样本，不是全量库。
    目的在于验证表结构、字段映射和对象序列化方案是否正确。
    """

    # Day 1 只写一个小型样例数据库，用来验证 schema 和对象映射是否正确。
    # 真正的全量入库放在 Day 2 再执行。
    if db_path.exists():
        db_path.unlink()
...
def main() -> None:
    ...
    schema_output_path.write_text(SCHEMA_PATH.read_text(encoding="utf-8"), encoding="utf-8")

    db_path = output_dir / "day1_sample.db"
