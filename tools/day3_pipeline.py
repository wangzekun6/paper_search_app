...
这个文件负责从 Day 2 的 SQLite 论文库中读取论文信息，
构造一个固定长度、固定字段的模型输入窗口，
调用大模型生成结构化的 PaperSemanticCard，
并把结果缓存回数据库和 day3_outputs 目录。
...
from day1_contracts import PaperSemanticCard
from day2_pipeline import DEFAULT_DB_PATH, connect_db
...
DAY2_QUERY_DEBUG_PATH = PROJECT_ROOT / "day2_outputs" / "query_debug_results.json"
OUTPUT_DIR = PROJECT_ROOT / "day3_outputs"
PROMPT_PATH = OUTPUT_DIR / "paper_semantic_card_prompt.md"
PILOT_OUTPUT_PATH = OUTPUT_DIR / "pilot_semantic_cards.json"
SAMPLE_OUTPUT_PATH = OUTPUT_DIR / "semantic_cards_sample_100.json"
QUALITY_CHECK_CSV_PATH = OUTPUT_DIR / "semantic_card_quality_check.csv"
QUALITY_CHECK_JSON_PATH = OUTPUT_DIR / "semantic_card_quality_check.json"
FIELD_STABILITY_PATH = OUTPUT_DIR / "semantic_card_field_stability.json"
CACHE_STRATEGY_PATH = OUTPUT_DIR / "card_cache_strategy.txt"
FEEDBACK_PATH = OUTPUT_DIR / "day3_feedback.txt"
ERROR_LOG_PATH = OUTPUT_DIR / "semantic_card_errors.json"

PROMPT_VERSION = "day3_v1"
DEFAULT_TARGET_COUNT = 100
DEFAULT_PILOT_COUNT = 5
DEFAULT_QUALITY_SAMPLE_SIZE = 20
GENERATED_STATUSES = ("generated", "generated_fallback")
...
def parse_args() -> argparse.Namespace:
    """解析 Day 3 命令行参数，支持批量生成和单篇重刷两种模式。"""

    parser = argparse.ArgumentParser(description="Generate Day 3 PaperSemanticCard objects with OpenAI.")
...
def heuristic_semantic_card(paper_context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    它不是高质量替代品，而是保证在 API 失败时系统仍能继续缓存和展示结果。
    """
...
def select_candidate_paper_ids(conn: Any, limit: int) -> List[str]:
    ...
    先取 Day 2 调试 query 中已经被命中的论文，再按较新的 paper_id 补齐，
    这样更符合“先覆盖高价值论文”的策略。
    """
    ...
    # 生成优先级分两层：
    # 先处理 Day 2 调试 query 里高频命中的论文，因为这些论文最可能先被用户搜到；
    # 不够时再按较新的 paper_id 继续补齐。
    if DAY2_QUERY_DEBUG_PATH.exists():
        payload = json.loads(DAY2_QUERY_DEBUG_PATH.read_text(encoding="utf-8"))
...
def generate_card_for_paper(conn: Any, paper_id: str, refresh: bool = False) -> Tuple[Optional[Dict[str, Any]], Optional[str]]:
    ...
    if not refresh:
        ...
        if existing and existing["card_status"] in GENERATED_STATUSES:
            return json.loads(existing["semantic_card_json"]), None
    ...
    try:
        # 优先走真实模型生成。
        # 只有当请求超时、返回异常或结构化输出失败时，才退回本地 heuristic 版本。
        ...
        upsert_semantic_card(conn, fallback_card, "generated_fallback")
...
后续扩展
- Day 4 / Day 5 可以对 top-K 命中的缺失卡片执行按需补生成。
...
- 已固定 LLM Prompt，并保存到 paper_semantic_card_prompt.md。
